...
- **Dalga 74 (tamamlandı):** **Bütçe Senaryo Simülatörü** (`/budget-simulator`) — `GET /budget-simulator/baseline` + `POST /budget-simulator/simulate`; kanallar arası bütçe kaydırma → geçmiş verimliliğe dayalı doğrusal projeksiyon (Gösterim/Tıklama/Dönüşüm/Gelir/ROAS) + baz karşılaştırma. Planlama persona'sının interaktif what-if aracı (yeni tablo yok).
- **Dalga 75 (tamamlandı):** **Rol Görünümü** (`/roles`) — `GET /role-views/roles` + `GET /role-views/{role}`; 5 persona (Performans/Marcom/Müşteri Hizmetleri/Planlama/Yönetim) için role-özel KPI + dikkat + öncelikli ekran + hızlı işlem kokpiti (mevcut okuma fonksiyonlarından sentez, yeni tablo yok). "Her ekip tek panelden kendi işi" vizyonunun çekirdeği.
- **Dalga 76 (tamamlandı):** **Müşteri Yolculuğu / Dönüşüm Hunisi** (`/funnel`) — `GET /funnel/overview`; ConversionEvent'ten 5 adımlı e-ticaret hunisi (adım dönüşümü + düşüş + en büyük düşüş). Görsel pazarlama analitiği farklılaştırıcısı (yeni tablo yok).
- **Dalga 77 (tamamlandı):** **Copilot modül farkındalığı** — "Veriye Sor" funnel/KVKK/kıyaslama/denetim sorularını da yanıtlar (4 yeni okuma aracı + 4 niyet dalı).
- **Dalga 78 (tamamlandı):** **Gruplu navigasyon** — 32→33 sayfa 7 kategori açılır menüde; masaüstü nav kırpılması (overflow:hidden) giderildi.
- **Dalga 79 (tamamlandı):** **Postgres deploy düzeltmesi** — `alembic upgrade head` Postgres'te kırıktı (GUID FK tip uyuşmazlığı + çift-tırnaklı server_default); düzeltildi, gerçek Postgres 16'da uçtan uca doğrulandı; opt-in `SEED_DEMO` demo seed (render.yaml).
- **Dalga 80 (tamamlandı):** **Pazarlama Fırsat Takvimi** (`/marketing-calendar`) — `GET /marketing-calendar/opportunities`; TR ticari/sezonsal günler + lead-time + içerik/bütçe hazırlık durumu + aksiyon CTA'ları. TR-first proaktif planlama farklılaştırıcısı (yeni tablo yok).
- **Dalga 81 (tamamlandı):** **Komuta Merkezi zenginleştirme** — vitrin modül durumu bloğuna Öneriler + KVKK Uyum + Dönüşüm Hunisi kartları eklendi (her biri dayanıklı try/except; mevcut bloklar değişmedi). "Tek panel" vitrini tüm ürünü yansıtıyor.
- **Dalga 82 (tamamlandı):** **Pazarlama Sağlık Endeksi** (`/health-index`) — `GET /health-index`; tüm pazarlamanın tek 0-100 stratejik puanı, 6 boyutu (hesap/sektör/KVKK/dönüşüm/hedef/bütçe) mevcut modüllerden sentezler. CMO/Yönetim farklılaştırıcısı (yeni tablo yok).
- **Dalga 77 (tamamlandı):** **Copilot modül farkındalığı** — "Veriye Sor" artık funnel/KVKK-rıza/kıyaslama/denetim sorularını yanıtlıyor (copilot_tools'a 4 yeni araç + copilot.py'ye 4 niyet dalı; yeni nav öğesi yok). Merkezi AI farklılaştırıcısı yeni modülleri kapsıyor.
